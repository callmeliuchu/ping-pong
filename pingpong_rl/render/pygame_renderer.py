...
class PygameRenderer:
    ...
    def render_gravity(self, env, mode: str = "human"):
        ...
        if hasattr(env, "rally_length"):
            if hasattr(env, "style_match_landings"):
            ...
            elif getattr(env, "robot_arm_bilateral_league_enabled", False):
                side = getattr(env, "controlled_side", "blue")
                gravity_mode = f"robot_bilateral_{side} {getattr(env, 'last_stroke_type', 'none')}"
            elif getattr(env, "robot_arm_grand_champion_attack_enabled", False):
                gravity_mode = f"robot_grand {getattr(env, 'last_stroke_type', 'none')}"
                ...
                gravity_mode = f"robot_adaptive {getattr(env, 'last_stroke_type', 'none')}"
            elif getattr(env, "robot_arm_attack_league_enabled", False):
                gravity_mode = f"robot_attack {getattr(env, 'last_stroke_type', 'none')}"
            elif getattr(env, "robot_arm_tactical_league_enabled", False):
                gravity_mode = f"robot_tactical {getattr(env, 'last_stroke_type', 'none')}"
            elif getattr(env, "robot_arm_league_enabled", False):
                gravity_mode = f"robot_league {getattr(env, 'last_stroke_type', 'none')}"
