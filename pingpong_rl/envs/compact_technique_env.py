...
class CompactTechniqueEnv(AdvancedStrokesEnv):
    ...
    def _apply_agent_action(self, action: np.ndarray) -> None:
        ...
        if self.ball_vx < 0:
            ...
            desired_angle = np.clip((target_y - self.agent_y) / 105.0 + swing_lift, -1.0, 1.0) * self.config.max_paddle_angle
        else:
            target_y = self.config.table_y - 102
            target_x = self.config.table_left - 32.0 + max(raw_attack, 0.0) * 38.0 + self.stroke_power * 16.0
            desired_angle = np.clip(0.22 * self.brush_intent + 0.20 * raw_attack, -0.58, 0.58)
    ...
    def _classify_stroke(self, speed: float, brush: float, power: float) -> str:
        angled_loop = abs(self.agent_angle) >= self.config.min_loop_angle
        angled_drive = abs(self.agent_angle) >= self.config.min_drive_angle
    ...
    def _bounce_from_paddle(self, player: str) -> None:
        ...
        if player != "agent" or before_hits is None:
            return
        angled = abs(self.agent_angle) >= self.config.min_drive_angle
    ...
    def _reward(
        ...
        if agent_hit and abs(self.agent_angle) >= self.config.min_drive_angle:
