...
from train.evaluate_robot_arm_bilateral_league import default_bilateral_opponent_paths


def main() -> None:
    parser = argparse.ArgumentParser(description="Watch the Stage 22 bilateral robot-arm league.")
    parser.add_argument("--model-path", type=Path, default=ROOT / "models" / "passed" / "ppo_stage22_blue.zip")
    ...
    if args.gif_path is not None:
        env = RobotArmBilateralLeagueEnv(render_mode="rgb_array", config=config)
    ...
    env = RobotArmBilateralLeagueEnv(render_mode="human", config=config)
    obs, _ = env.reset(seed=args.seed)
    while True:
        action, _ = model.predict(obs, deterministic=True)
        obs, _, terminated, truncated, _ = env.step(action)
