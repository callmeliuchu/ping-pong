...
def main() -> None:
    parser = argparse.ArgumentParser(description="Watch the Stage 19 clean adaptive attacking robot-arm league policy.")
    parser.add_argument("--model-path", type=Path, default=ROOT / "models" / "passed" / "ppo_stage19.zip")
    ...
    if args.gif_path is not None:
        env = RobotArmCleanAdaptiveAttackEnv(render_mode="rgb_array", config=config)
    ...
    env = RobotArmCleanAdaptiveAttackEnv(render_mode="human", config=config)
    obs, _ = env.reset(seed=args.seed)
    while True:
        action, _ = model.predict(obs, deterministic=True)
        obs, _, terminated, truncated, _ = env.step(action)
