...
def main() -> None:
    parser = argparse.ArgumentParser(description="Watch the Stage 21 grand champion robot-arm league policy.")
    parser.add_argument("--model-path", type=Path, default=ROOT / "models" / "passed" / "ppo_stage21.zip")
    ...
    if args.gif_path is not None:
        env = RobotArmGrandChampionAttackEnv(render_mode="rgb_array", config=config)
    ...
    env = RobotArmGrandChampionAttackEnv(render_mode="human", config=config)
    obs, _ = env.reset(seed=args.seed)
    while True:
        action, _ = model.predict(obs, deterministic=True)
        obs, _, terminated, truncated, _ = env.step(action)
