...
def default_robot_arm_opponent_paths() -> list[Path]:
    candidates = [
        ROOT / "models" / "passed" / "ppo_stage14",
        ROOT / "models" / "ppo_robot_arm_stage14_spin_v2",
        ROOT / "models" / "best" / "stage14" / "best_model",
    ]
    candidates.extend(sorted((ROOT / "models" / "selfplay" / "stage15").glob("gen_*.zip")))
    paths: list[Path] = []
    seen: set[Path] = set()
    for candidate in candidates:
        source = candidate if candidate.suffix == ".zip" else candidate.with_suffix(".zip")
        if not _zip_exists(candidate):
            continue
...
def _evaluate_one(model: PPO, opponent_model_path: Path, episodes: int, seed: int) -> dict[str, Any]:
    config = RobotArmLeagueConfig(opponent_model_paths=(str(opponent_model_path),))
    env = RobotArmLeagueEnv(config=config)
    totals = {
        "terminated": 0,
        "truncated": 0,
        "scores": 0,
    ...
    for episode in range(episodes):
        ...
        totals["red_loop_episodes"] += int(info.get("red_loop_attempts", 0) > 0)
        totals["red_drive_episodes"] += int(info.get("red_drive_attempts", 0) > 0)
        totals["red_topspin_episodes"] += int(info.get("red_topspin_attempts", 0) > 0)
    ...
    return {
        ...
        "red_loop_attempt_rate": totals["red_loop_episodes"] / episodes,
        "red_drive_attempt_rate": totals["red_drive_episodes"] / episodes,
        "red_topspin_attempt_rate": totals["red_topspin_episodes"] / episodes,
...
def evaluate_robot_arm_league_model(
    model_path: Path,
    ...
    if not per_opponent:
        raise ValueError("Stage 15 league evaluation needs at least one robot-arm opponent model.")
    ...
    return {
        ...
        "backspin_landing_rate": avg("backspin_landing_rate"),
        ...
        "red_loop_attempt_rate": avg("red_loop_attempt_rate"),
        "red_drive_attempt_rate": avg("red_drive_attempt_rate"),
        "red_topspin_attempt_rate": avg("red_topspin_attempt_rate"),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Stage 15 robot-arm league self-play.")
    parser.add_argument("--model-path", type=Path, default=ROOT / "models" / "passed" / "ppo_stage15")
    ...
    metrics = evaluate_robot_arm_league_model(args.model_path, opponent_paths, args.episodes, seed=args.seed)
