...
from train.evaluate_robot_arm_clean_adaptive_attack import default_clean_adaptive_opponent_paths


def _label(path: Path) -> str:
    if path.parent.name == "passed":
        return path.stem
    if path.parent.name in {"stage15", "stage16", "stage17", "stage18", "stage19", "stage20"}:
...
def default_champion_attack_opponent_paths() -> list[Path]:
    candidates = default_clean_adaptive_opponent_paths()
    candidates.append(ROOT / "models" / "passed" / "ppo_stage19")
    candidates.extend(sorted((ROOT / "models" / "selfplay" / "stage20").glob("gen_*.zip")))
    paths: list[Path] = []
    seen: set[Path] = set()
...
def evaluate_robot_arm_champion_attack_model(
    model_path: Path,
    ...
    profiles = RobotArmChampionAttackConfig().pressure_profiles
    ...
    if not per_match:
        raise ValueError("Stage 20 champion attack evaluation needs at least one robot-arm opponent model.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Stage 20 champion clean-finishing robot-arm league.")
    parser.add_argument("--model-path", type=Path, default=ROOT / "models" / "passed" / "ppo_stage20")
    ...
    opponent_paths = args.opponent_model_paths if args.opponent_model_paths is not None else default_champion_attack_opponent_paths()
    metrics = evaluate_robot_arm_champion_attack_model(args.model_path, opponent_paths, args.episodes, seed=args.seed)
