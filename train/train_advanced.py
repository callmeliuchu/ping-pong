...
def make_env():
    def _init():
        return Monitor(AdvancedStrokesEnv(render_mode=None))

    return _init


def main() -> None:
    parser = argparse.ArgumentParser(description="Train Stage 9 advanced strokes with PPO.")
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="ppo_advanced_stage9",
    )
    eval_env = Monitor(AdvancedStrokesEnv(render_mode=None))
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=str(ROOT / "models" / "best" / "stage9"),
        log_path=str(ROOT / "logs" / "ppo_advanced_stage9" / "eval"),
    ...
    if args.load_model_path is not None:
        ...
        model = PPO(
            ...
            verbose=1,
            seed=args.seed,
            tensorboard_log=str(ROOT / "logs" / "ppo_advanced_stage9"),
        )
    ...
    metrics = evaluate_advanced_model(args.model_path, args.eval_episodes, seed=args.seed)
    write_metrics(metrics, args.eval_json_path or ROOT / "logs" / "ppo_advanced_stage9" / "final_eval.json")
