...
def default_red_scoring_bilateral_opponent_paths() -> list[Path]:
    candidates = default_bilateral_opponent_paths()
    candidates.extend(
        [
            ROOT / "models" / "passed" / "ppo_stage23_blue",
            ROOT / "models" / "passed" / "ppo_stage23_red",
        ]
    )
    candidates.extend(sorted((ROOT / "models" / "selfplay" / "stage23").glob("blue_gen_*.zip")))
    candidates.extend(sorted((ROOT / "models" / "selfplay" / "stage23").glob("red_gen_*.zip")))
...
def _evaluate_blue_one(model: PPO, opponent_model_path: Path, profile_index: int, episodes: int, seed: int) -> dict[str, Any]:
    ...
    env = RobotArmRedScoringBilateralEnv(config=config)
    totals = {
        "terminated": 0,
        "truncated": 0,
        "blue_scores": 0,
...
def evaluate_blue_red_scoring_bilateral_model(
    model_path: Path,
    ...
    profiles = RobotArmRedScoringBilateralConfig().pressure_profiles
    ...
    if not per_match:
        raise ValueError("Stage 23 red-scoring bilateral evaluation needs at least one opponent model.")
...
def evaluate_red_scoring_challenge(
    red_model_path: Path,
...
def red_scoring_bilateral_score(metrics: dict[str, Any]) -> float:
    return bilateral_score(metrics) + 40.0 * float(metrics.get("red_win_rate", 0.0))


def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Stage 23 red-scoring bilateral robot-arm league.")
    parser.add_argument("--model-path", type=Path, default=ROOT / "models" / "passed" / "ppo_stage23")
    ...
    parser.add_argument("--blue-model-path", type=Path, default=ROOT / "models" / "passed" / "ppo_stage23_blue")
    ...
    if args.red_challenge_model_path is not None:
        metrics = evaluate_red_scoring_challenge(
            args.red_challenge_model_path,
            args.blue_model_path,
            args.episodes,
            seed=args.seed,
        )
    else:
        opponent_paths = (
            ...
            else default_red_scoring_bilateral_opponent_paths()
        )
        metrics = evaluate_blue_red_scoring_bilateral_model(args.model_path, opponent_paths, args.episodes, seed=args.seed)
        metrics["bilateral_score"] = red_scoring_bilateral_score(metrics)
