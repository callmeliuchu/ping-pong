...
def make_env():
    def _init():
        return Monitor(RobotArmPingPongEnv(render_mode=None))

    return _init


def main() -> None:
    parser = argparse.ArgumentParser(description="Train Stage 14 joint-controlled robot-arm ping pong with PPO.")
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="ppo_robot_arm_stage14",
    )
    eval_env = Monitor(RobotArmPingPongEnv(render_mode=None))
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=str(ROOT / "models" / "best" / "stage14"),
        log_path=str(ROOT / "logs" / "ppo_robot_arm_stage14" / "eval"),
    ...
    if args.load_model_path is not None:
        ...
        model = PPO(
            ...
            verbose=1,
            seed=args.seed,
            tensorboard_log=str(ROOT / "logs" / "ppo_robot_arm_stage14"),
        )
    ...
    metrics = evaluate_robot_arm_model(args.model_path, args.eval_episodes, seed=args.seed)
    write_metrics(metrics, args.eval_json_path or ROOT / "logs" / "ppo_robot_arm_stage14" / "final_eval.json")
