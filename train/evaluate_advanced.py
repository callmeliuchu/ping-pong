...
def evaluate_advanced_model(
    ...
    env = AdvancedStrokesEnv(render_mode="human" if render else None)
    ...
    totals = {
        ...
        "outside_play": 0,
        "outside_hits": 0,
        "loop_attempt_episodes": 0,
        "loop_landing_episodes": 0,
        "drive_attempt_episodes": 0,
    ...
    for episode in range(episodes):
        ...
        totals["outside_play"] += int(info["outside_play"])
        totals["outside_hits"] += int(info["outside_hits"] > 0)
        totals["loop_attempt_episodes"] += int(info["loop_attempts"] > 0)
        totals["loop_landing_episodes"] += int(info["loop_landings"] > 0)
        totals["drive_attempt_episodes"] += int(info["drive_attempts"] > 0)
    ...
    return {
        ...
        "outside_play_rate": totals["outside_play"] / episodes,
        "outside_hit_rate": totals["outside_hits"] / episodes,
        "loop_attempt_rate": totals["loop_attempt_episodes"] / episodes,
        "loop_landing_rate": totals["loop_landing_episodes"] / episodes,
        "drive_attempt_rate": totals["drive_attempt_episodes"] / episodes,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate the Stage 9 advanced-strokes policy.")
    parser.add_argument("--model-path", type=Path, default=ROOT / "models" / "ppo_advanced_stage9")
    ...
    metrics = evaluate_advanced_model(args.model_path, args.episodes, args.render, args.seed)
