...
def make_env():
    def _init():
        return Monitor(CompetitiveRealisticEnv(render_mode=None))

    return _init


def main() -> None:
    parser = argparse.ArgumentParser(description="Train Stage 8 competitive realistic ping pong with PPO.")
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="ppo_competitive_stage8",
    )
    eval_env = Monitor(CompetitiveRealisticEnv(render_mode=None))
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=str(ROOT / "models" / "best" / "stage8"),
        log_path=str(ROOT / "logs" / "ppo_competitive_stage8" / "eval"),
    ...
    if args.load_model_path is not None:
        ...
        model = PPO(
            ...
            verbose=1,
            seed=args.seed,
            tensorboard_log=str(ROOT / "logs" / "ppo_competitive_stage8"),
        )
    ...
    metrics = evaluate_competitive_model(args.model_path, args.eval_episodes, seed=args.seed)
    write_metrics(metrics, args.eval_json_path or ROOT / "logs" / "ppo_competitive_stage8" / "final_eval.json")
