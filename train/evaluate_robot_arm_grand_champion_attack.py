...
from train.evaluate_robot_arm_champion_attack import default_champion_attack_opponent_paths


def _label(path: Path) -> str:
    if path.parent.name == "passed":
        return path.stem
    if path.parent.name in {"stage15", "stage16", "stage17", "stage18", "stage19", "stage20", "stage21"}:
...
def _is_stage20_champion(path: Path) -> bool:
    source = path if path.suffix == ".zip" else path.with_suffix(".zip")
    champion = ROOT / "models" / "passed" / "ppo_stage20.zip"
    return source.exists() and champion.exists() and source.resolve() == champion.resolve()


def default_grand_champion_attack_opponent_paths() -> list[Path]:
    candidates = default_champion_attack_opponent_paths()
    candidates.append(ROOT / "models" / "passed" / "ppo_stage20")
    candidates.extend(sorted((ROOT / "models" / "selfplay" / "stage21").glob("gen_*.zip")))
    paths: list[Path] = []
    seen: set[Path] = set()
...
def _evaluate_one(model: PPO, opponent_model_path: Path, profile_index: int, episodes: int, seed: int) -> dict[str, Any]:
    config = RobotArmGrandChampionAttackConfig(
        opponent_model_paths=(str(opponent_model_path),),
        pressure_profile_index=profile_index,
    )
    env = RobotArmGrandChampionAttackEnv(config=config)
    totals = {
        "terminated": 0,
        "truncated": 0,
        "scores": 0,
...
def evaluate_robot_arm_grand_champion_attack_model(
    model_path: Path,
    ...
    profiles = RobotArmGrandChampionAttackConfig().pressure_profiles
    per_match: dict[str, dict[str, Any]] = {}
    champion_match_names: set[str] = set()
    for profile_index, profile in enumerate(profiles):
        for opponent_index, opponent_path in enumerate(opponent_model_paths):
            name = f"{profile[0]}:{_label(opponent_path)}"
            if _is_stage20_champion(opponent_path):
                champion_match_names.add(name)
    ...
    if not per_match:
        raise ValueError("Stage 21 grand champion evaluation needs at least one robot-arm opponent model.")

    def avg(key: str, names: set[str] | None = None) -> float:
        matches = [metrics for name, metrics in per_match.items() if names is None or name in names]
        return sum(float(metrics[key]) for metrics in matches) / len(matches) if matches else 0.0
    ...
    return {
        ...
        "champion_matchup_count": len(champion_match_names),
        "champion_win_rate": avg("win_rate", champion_match_names),
        "champion_forced_finish_rate": avg("forced_finish_rate", champion_match_names),
        "champion_stalemate_rate": avg("stalemate_rate", champion_match_names),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Stage 21 grand champion robot-arm league.")
    parser.add_argument("--model-path", type=Path, default=ROOT / "models" / "passed" / "ppo_stage21")
    ...
    opponent_paths = (
        args.opponent_model_paths if args.opponent_model_paths is not None else default_grand_champion_attack_opponent_paths()
    )
    metrics = evaluate_robot_arm_grand_champion_attack_model(args.model_path, opponent_paths, args.episodes, seed=args.seed)
