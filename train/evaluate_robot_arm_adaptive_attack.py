...
from train.evaluate_robot_arm_attack import default_attack_opponent_paths


def _label(path: Path) -> str:
    if path.parent.name == "passed":
        return path.stem
    if path.parent.name in {"stage15", "stage16", "stage17", "stage18"}:
...
def default_adaptive_opponent_paths() -> list[Path]:
    candidates = default_attack_opponent_paths()
    candidates.extend(sorted((ROOT / "models" / "selfplay" / "stage18").glob("gen_*.zip")))
    paths: list[Path] = []
    seen: set[Path] = set()
...
def _evaluate_one(model: PPO, opponent_model_path: Path, profile_index: int, episodes: int, seed: int) -> dict[str, Any]:
    ...
    env = RobotArmAdaptiveAttackEnv(config=config)
    totals = {
        "terminated": 0,
        "truncated": 0,
        "scores": 0,
...
def evaluate_robot_arm_adaptive_attack_model(
    model_path: Path,
    ...
    profiles = RobotArmAdaptiveAttackConfig().pressure_profiles
    ...
    if not per_match:
        raise ValueError("Stage 18 adaptive attack evaluation needs at least one robot-arm opponent model.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Stage 18 adaptive attacking robot-arm league.")
    parser.add_argument("--model-path", type=Path, default=ROOT / "models" / "passed" / "ppo_stage18")
    ...
    metrics = evaluate_robot_arm_adaptive_attack_model(args.model_path, opponent_paths, args.episodes, seed=args.seed)
