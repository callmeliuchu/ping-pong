...
def make_env():
    def _init():
        return Monitor(GravityPingPongEnv(render_mode=None))

    return _init


def main() -> None:
    parser = argparse.ArgumentParser(description="Train Stage 6 gravity ping pong with PPO.")
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="ppo_gravity_stage6",
    )
    eval_env = Monitor(GravityPingPongEnv(render_mode=None))
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=str(ROOT / "models" / "best" / "stage6"),
        log_path=str(ROOT / "logs" / "ppo_gravity_stage6" / "eval"),
    ...
    if args.load_model_path is not None:
        ...
        model = PPO(
            ...
            verbose=1,
            seed=args.seed,
            tensorboard_log=str(ROOT / "logs" / "ppo_gravity_stage6"),
        )
    ...
    metrics = evaluate_gravity_model(args.model_path, args.eval_episodes, seed=args.seed)
    write_metrics(metrics, args.eval_json_path or ROOT / "logs" / "ppo_gravity_stage6" / "final_eval.json")
