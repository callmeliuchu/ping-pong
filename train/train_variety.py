...
def make_env():
    def _init():
        return Monitor(VarietyTechniqueEnv(render_mode=None))

    return _init


def main() -> None:
    parser = argparse.ArgumentParser(description="Train Stage 11 varied-stroke technique with PPO.")
    parser.add_argument("--timesteps", type=int, default=300_000)
    parser.add_argument("--num-envs", type=int, default=8)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--model-path", type=Path, default=ROOT / "models" / "ppo_variety_stage11")
    parser.add_argument("--load-model-path", type=Path, default=ROOT / "models" / "passed" / "ppo_stage10")
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="ppo_variety_stage11",
    )
    eval_env = Monitor(VarietyTechniqueEnv(render_mode=None))
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=str(ROOT / "models" / "best" / "stage11"),
        log_path=str(ROOT / "logs" / "ppo_variety_stage11" / "eval"),
    ...
    if args.load_model_path is not None:
        ...
        model = PPO(
            ...
            verbose=1,
            seed=args.seed,
            tensorboard_log=str(ROOT / "logs" / "ppo_variety_stage11"),
        )
    ...
    metrics = evaluate_variety_model(args.model_path, args.eval_episodes, seed=args.seed)
    write_metrics(metrics, args.eval_json_path or ROOT / "logs" / "ppo_variety_stage11" / "final_eval.json")
